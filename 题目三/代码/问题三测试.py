...
import matplotlib
matplotlib.use('Agg')
import time, random, numpy as np, pandas as pd, torch, torch.nn as nn, math
...
def load_data(path: str):
    ...
    df['nipt_day'] = df['检测孕周']          # 统一为「天」

    ### K-means 分组 ###
    kmeans = KMeans(n_clusters=GROUP_NUM, random_state=42, n_init='auto')
    df['group_id'] = kmeans.fit_predict(df[['孕妇BMI']])
    # 生成边界表：按聚类中心排序，保证组号 0→4 与 BMI 单调递增
    centers = kmeans.cluster_centers_.ravel()
    order = np.argsort(centers)          # 中心值升序对应的组号
    df['group_id'] = df['group_id'].map({o: i for i, o in enumerate(order)})
    # 计算每组的 [min, max] 用于后续规则保存
    edges = (df.groupby('group_id')['孕妇BMI']
               .agg(['min', 'max'])
...
class MLPRegClf(nn.Module):
    def __init__(self, input_dim=5, hidden=64, n_groups=GROUP_NUM):
        super().__init__()
...
for epoch in range(1, EPOCHS+1):
    ...
    model.train()
    epoch_tr_loss = 0.
    tr_true_cls = []; tr_pred_cls = []; tr_true_reg = []; tr_pred_reg = []
    ...
    model.eval()
    epoch_val_loss = 0.
    val_true_cls = []; val_pred_cls = []; val_true_reg = []; val_pred_reg = []
    with torch.no_grad():
...
plt.title('Loss / Epoch'); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend(); plt.grid(True)
...
plt.title('Accuracy / Epoch'); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend(); plt.grid(True)

plt.subplot(3, 1, 3)
plt.plot(epochs_range, history['train_mae'], label='Train MAE', color=colors[4], linewidth=2)
plt.plot(epochs_range, history['val_mae'], label='Val MAE', color='tab:brown', linewidth=2)
plt.title('MAE (days) / Epoch'); plt.xlabel('Epoch'); plt.ylabel('MAE (days)'); plt.legend(); plt.grid(True)
...
cm = confusion_matrix(y_clf_true, y_clf_pred,
                      labels=list(range(GROUP_NUM)))   # 5×5
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=range(GROUP_NUM),
            yticklabels=range(GROUP_NUM))
